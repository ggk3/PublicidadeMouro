...
if(isset($_GET['busca'])){
	$sql = "SELECT * FROM produtos WHERE descricao LIKE '%".$_GET['busca']."%' ORDER BY ativo DESC";
}else{
	$sql = "SELECT * FROM produtos ORDER BY ativo DESC";
}
...
<nav class="navbar navbar-inverse navbar-custom navbar-fixed-top">
	...
	<!-- header -->
		...
		<ul class="nav navbar-nav ">
	            ...
	            <li  class="active"><a href="painel_ativos.php">Painel Indoor</a></li>
...
<div class="container">
	<div class="col-md-12">
		...
		<form action="painel_ativos.php" method="get">
			<div class="col-md-4">
				<input type="text" name="busca" class="form-control">
			</div>
			<div class="col-md-4">
				<button class="btn btn-success">Buscar</button>
				<a href="painel_ativos.php" class="btn btn-warning">Limpar Busca</a>
				
			</div>
		</form>
		<form method="post" action="atualiza_nomes.php?para=0">
		<div class="col-md-4">
		<button class="btn btn-primary">Atualizar Nomes</button>
			...
			<a href="limpar_ativos.php?para=0" class="btn btn-danger">Limpar Ativos</a>
		</div>
	...
	if($resultado_id){
		while($produto = mysqli_fetch_array($resultado_id)){
			...
			echo "<td>R$ " . $preco_item . "</td>";

			if($produto['ativo'] == 0){
				echo "<td align='center' style='width: 20%;'> " . "<a href='ativar.php?para=0&id=".$produto['id_produto']."'>Ativar</a>" . "</td>";
			}else{
				echo "<td align='center' style='width: 20%;'> " . "<a href='desativar.php?para=0&id=".$produto['id_produto']."'>Desativar</a>" . "</td>";
			}
			echo "</tr>";
